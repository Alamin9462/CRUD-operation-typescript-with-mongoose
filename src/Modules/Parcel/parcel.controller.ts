...
export const getParcelMetrics = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const dailyBookings = await parcelService.getParcels({
      createdAt: { $gte: today, $lt: tomorrow },
    });
    const failedDeliveries = await parcelService.getParcels({
      status: "Failed",
    });
...
export const exportParcelsCSV = catchAsync(
  async (req: Request, res: Response) => {
...
export const updateParcelLocation = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const parcel = await parcelService.updateParcel(req.params.id, {
...
export const getParcelLocation = catchAsync(
  async (req: Request, res: Response) => {
    const user = (req as any).user;
    if (!user || user.role !== "customer" || user.role !== "delivery_agent") {
      return sendResponse(res, {
        statusCode: httpStatus.FORBIDDEN,
        success: false,
        message: "Only customers and Agent can track parcel location",
      });
    }
...
export const getPickupLocationsByAgent = async (req: Request, res: Response) => {
  try {
    ...
    const parcels = await Parcel.find({ agent: agentId }).select('pickupAddress currentLocation');
    console.log("Parcels found:", parcels);

    const pickups = parcels.map(parcel => ({
      parcelId: parcel._id,
      address: parcel.pickupAddress,
      lat: parcel.currentLocation?.lat ?? null,
      lng: parcel.currentLocation?.lng ?? null,
    }));
...
export const updateParcelStatus = catchAsync(
  async (req: Request, res: Response) => {
    const user = (req as any).user;
    if (!user || user.role !== "delivery_agent") {
      return sendResponse(res, {
...
export const createParcel = catchAsync(async (req: Request, res: Response) => {
  ...
  console.log("Creating parcel for customer:", customerId);
  const parcelData = {
    ...req.body,
    customer: customerId,
  };
...
// export const getAssignedParcels= catchAsync(async (req: Request, res: Response) => {
//   const parcel = await parcelService.getParcelById(req.params.id);
//   if (!parcel) {
//     return sendResponse(res, {
//       statusCode: httpStatus.NOT_FOUND,
...
//       statusCode: httpStatus.FORBIDDEN,
//       success: false,
//       message: 'Only agents can view assigned parcels',
//     });
//   }

//   const parcels = await parcelService.getParcels({ agent: user.id });
